...
if(n.outcome.hl>0) {
  for (out in 1:n.outcome.hl) {
    ...
    RR_pooled[out] = round(exp(meta_high_low$TE.fixed),2)
    p_RR0[out]     = meta_high_low$pval.fixed         
    lb[out] = round(exp(meta_high_low$lower.fixed),2)
    ub[out] = round(exp(meta_high_low$upper.fixed),2)
    ...
    if (length(unique(high_low$id))>2) {
      ET = eggers.test(x = meta_high_low)
...
if (n.outcome.hl>0) {
  ...
  if (n.outcome.hl>1) {
    for (out in 1:(length(outcomes.hl)-1)) {
      cat(association_direction[out], " association between ",tolower(curr.exposure)," intake and risk of ",
          outcomes.hl[out], " was observed (RR: ",RR_pooled[out],"; 95% CI ",lb[out]," to ",ub[out],"), ",sep="")
    ...
    cat("and ", association_direction[out], " association between ",tolower(curr.exposure)," intake and risk of ",
        outcomes.hl[out], " was observed (RR: ",RR_pooled[out],"; 95% CI ",lb[out]," to ",ub[out],
  ...
  if (n.outcome.hl == 1) {
    ...
    out = length(outcomes.hl)
    cat(" ", association_direction[out], " association between ",tolower(curr.exposure)," intake and risk of ",
        outcomes.hl[out]," was observed (RR: ",RR_pooled[out],"; 95% CI ",lb[out]," to ",ub[out], "). See figure ",curr.exposure.num,"_1_",1,".",sep="")
  }
}
...
if (n_outcome_lin>0) {
  for (out in 1:n_outcome_lin) {
    ...
    # Fixed effect meta-analyse (Note sm="RR" since all summary measures are interpreted as risk ratios.)
    ...
    RR_pooled_lin[out] = round(exp(meta_extra_portion$TE.fixed),2)
    p_RR0_lin[out]     = meta_extra_portion$pval.fixed
    lb_lin[out] = round(exp(meta_extra_portion$lower.fixed),2)
    ub_lin[out] = round(exp(meta_extra_portion$upper.fixed),2)
    ...
    if (length(unique(data.doseresponse$id))>2) {
      ET = eggers.test(x = meta_extra_portion)
  ...
  for (out in 1:n_outcome_nonlin) {
    ...
    spl_model =  data.nonlin %>%
      dosresmeta(logRR ~ rcs(explevel, knots_bin), id = author.sex, se = logse, type = tolower(type),
                 cases = cases_exp, n = n_exp, data = .,covariance = "gl",method="fixed")
  ...
  if (n_lin >1) {
    for (lin in index_lin[1:(length(index_lin)-1)])
      cat(association_direction_lin[lin]," associated with risk of ",outcomes_lin[lin]," (RR: ",RR_pooled_lin[lin],"; 95% CI ",
          lb_lin[lin]," to ",ub_lin[lin],", n = ",unlist(n.studies.lin[which(n.studies.lin$outcome==outcomes_lin[lin]),"count"]),"), ",sep="")
